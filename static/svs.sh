...
PROGSFILE="https://raw.githubusercontent.com/sekigetsu01/SVS/main/static/progs.csv"
AURHELPER="yay"
BRANCH="main"
export TERM=ansi
...
info() { whiptail --title "SVS" --infobox "$1" 8 70; }


# DIALOG PROMPTS

welcome() {
	whiptail --title "Welcome to SVS!" \
		--msgbox "Welcome to Sekigetsu's VM Installer.\n\nThis script will automatically set up a full desktop environment.\n\n-Sekigetsu" 10 60
...
check_existing_user() {
	id -u "$name" >/dev/null 2>&1 && \
		whiptail --title "User already exists" \
			...
			--yesno "The user '$name' already exists.\n\nSVS will overwrite conflicting dotfiles and settings but will NOT touch personal files (Documents, Videos, etc).\n\nThe password will also be updated." 12 65
}
...
aur_install() {
	...
	whiptail --title "SVS – Installing ($n/$total)" --infobox "AUR: $1\n$2" 8 70
...
git_make_install() {
	...
	whiptail --title "SVS – Installing ($n/$total)" --infobox "git+make: $progname\n$2" 8 70
...
pipx_install() {
	...
	whiptail --title "SVS – Installing ($n/$total)" --infobox "pipx: $1\n$2" 8 70
...
flatpak_install() {
	whiptail --title "SVS – Installing ($n/$total)" --infobox "flatpak: $1\n$2" 8 70
	pkg flatpak
...
pacman_install() {
	whiptail --title "SVS – Installing ($n/$total)" --infobox "pacman: $1\n$2" 8 70
	pkg "$1"
}
...
trap 'rm -f /etc/sudoers.d/svs-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/svs-temp
